...
export interface Config {
  ...
  cache: { enableApiCache: boolean; apiCacheTTL: number; };
...
export const Config: Schema<Config> = Schema.intersect([
  ...
  Schema.object({
    ...
    promptTimeout: Schema.number().step(1000).min(1000).default(60000).description('交互式操作的等待超时时间 (毫秒)。'),
    imageSendDelay: Schema.number().step(1000).min(0).default(1000).description('以图片形式发送时，每张图片的发送间隔 (毫秒)。'),
  }).description('消息与交互'),
  ...
  Schema.object({
    ...
    downloadTimeout: Schema.number().min(1000).default(10000).step(1000).description('单张图片下载的超时时间 (毫秒)。'),
    downloadRetries: Schema.number().min(0).max(5).step(1).default(3).description('下载失败后的重试次数。'),
    downloadRetryDelay: Schema.number().min(0).step(1000).default(1000).description('每次重试前的等待时间 (毫秒)。'),
    cache: Schema.object({
